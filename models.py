...
class ArtificialNeuralNetwork:
  ...
  def activation_function(self, z: np.ndarray, is_grad: bool, act_func: str) -> np.ndarray:
    """
    ...
    if act_func not in mapper:
      raise Exception('Invalid activation function')
    
    return mapper[act_func](z, is_grad)
  ...
  def cost_function(self, X: np.ndarray, Y: np.ndarray):
    ...
    if self.loss == 'mse':
      J = loss.mse(A[-1], Y, X.shape[1])
    elif self.loss == 'bce':
      J = loss.bce(A[-1], Y, X.shape[1])
    else:
      J = loss.ce(X, Y)
