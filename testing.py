...
if __name__ == '__main__':
  ...
  while True:
    ...
    if loss in ['mse', 'bce', 'ce']:
      break
    elif loss == 'list':
      print('Valid Loss Functions: mse, bce, ce')
    ...
    if add_layer == 'n':
      ...
      while True:
        ...
        if activation_function == 'list':
          ...
          continue
        elif activation_function in ['sigmoid', 'tanh', 'relu', 'leaky_relu', 'linear', 'softmax']:
          break
      while True:
        add_dropout = input('Do you want to add dropout regularization? (y/n) ')
        if add_dropout in ['y', 'n']:
          break
      if add_dropout == 'y':
        while True:
