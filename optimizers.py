...
def Gradient_Descent(X, y, model, epochs, batch_size, alpha_0=1e-2, learning_decay_rate_type=None, discrete_reduction=0.001, decay_rate=1, k=1, metrics=[], show=False):
    ...
    \t> model => Object model of the neural network which parameters wants to be optimized.
    ...
    \t\t- 'inverse'
    \t\t- 'discrete'
    \t\t- if pass any diferent value default alpha-0 set as the learning rate
    \t> discrete-reduction => If learning-decay-rate-type is chose to be 'discrete', float value to reduce the value of the learning rate at the beginning of each epoch.
    \t> decay-rate => If learning-decay-rate-type is chose to be 'inverse', float value that defines the decay rate that redices the learning rate each epoch.
    \t> k => If learning-decay-rate-type is chose to be 'inverse-radical', float value that defines inversely how fast radicaly the learning decays in each epoch.
    \t> metrics => List of strings that define the metrics that wants to be tracked through the optimization algorithm, for now it can only be 'loss' or 'acc'
    \t> show => Boolean values that defines if the metrics and loading bar for the batches are shown in screen
    Returns:\n
    \t> model => Modified model after all the epochs have been runned.
    \t> J-history => The recording values of all the losses evaluated in each epoch after running through all the batches.
    \t> acc-history => The recording values of all the accuracy evaluated in each epoch after running through all the batches.
    ...
    * Note: variable names do not use - but underscore, the change is because underscore means something in pydocs jeje
    ...
    for i in range(epochs):
        ...
        for t in range(X.shape[1] // batch_size):
            ...
            if show == True: progress_bar(t+1, X.shape[1] // batch_size, length=30)
        ...
        if ('accuracy' in metrics or 'acc' in metrics) and show == True: 
            if model.loss == 'bce' or model.loss == 'ce': acc = np.mean(model.forward_propagation(X)[0][-1] == y)
            else: pass
...
def Adam(X, y, model, epochs, batch_size, alpha_0=1e-2, learning_decay_rate_type=None, discrete_reduction=0.001, decay_rate=1, k=1, beta1=0.9, beta2=0.999, momentum=0, epsilon=1e-8, metrics=[], show=False):
    ...
    For further reference check the paper => Adam: A Method for Stochastic Optimization => link to paper: https://arxiv.org/pdf/1412.6980.pdf \n
    Arguments:\n
    \t> X => Array like with the input values of the examples.
    \t> Y => Array like with the output values of the examples.
    \t> model => Object model of the neural network which parameters wants to be optimized.
    ...
    \t> epsilon => Value to control, when applying RMSProp, no to divide by 0
    \t> metrics => List of strings that define the metrics that wants to be tracked through the optimization algorithm, for now it can only be 'loss' or 'acc'
    \t> show => Boolean values that defines if the metrics and loading bar for the batches are shown in screen
    Returns:\n
    \t> model => Modified model after all the epochs have been runned.
